<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Sami Shop</title>
    <link rel="shortcut icon" type="image/x-icon" href="img/LOGO B.png">
    <link rel="stylesheet" href="estilos.css">
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.3.1/css/all.css" integrity="sha384-mzrmE5qonljUremFsqc01SB46JvROS7bZs3IO2EmfFsd15uHvIt+Y8vEf7N7fWAU"
        crossorigin="anonymous">
    <link rel="stylesheet" href="font.css">


    

</head>

<body class="hidden">
    <div class="centrado" id="onload">
        <div class="lds-facebook"><div></div><div></div><div></div></div>
    </div>

    <div class="social-bar">
        <a href="https://www.facebook.com/samishop1.peru" class="icon icon-facebook2" target="_blank" ></a>
        <a href="https://wa.me/51934504582?text=Hola,%20estoy%20interesado!" class="icon icon-whatsapp" target="_blank" ></a>
        <a href="https://www.instagram.com/samishop.peru/" class="icon icon-instagram" target="_blank" ></a>
        <a href="https://vm.tiktok.com/ZSxvcV8r/" class="icon icon-tiktok" target="_blank"></a>
    </div>

    <header>
        <nav id="nav" class="nav1">
            <div class="contenedor-nav">
                <div class="logo">
                    <img src="img/LOGOO_-removebg-preview.png" alt="">
                </div>
                <div class="enlaces" id="enlaces">
                    <a href="#" id="enlace-inicio" class="btn-header">Inicio</a>
                    <a href="#" id="enlace-trabajo" class="btn-header">Productos</a>
                    <a href="#" id="enlace-ofertas" classs="btn-header">Ofertas</a>
                    <a href="#" id="enlace-equipo" class="btn-header">Equipo</a>
                    <a href="#" id="enlace-servicio" class="btn-header">Servicios</a>
               
                </div>
                <div class="icono" id="open">
                    <span>&#9776;</span>
                </div>
            </div>
        </nav>
        <div class="textos">
            <h1>Sami Shop</h1>
            <h2>Tu tienda de ropas virtual</h2>
        </div>
    </header>
    <main>

<!--Esta seccion es de NUESTROS PRODUCTOS-->

        <section class="work contenedor" id="trabajo">
            <h3>Nuestros Productos</h3>
            <p class="after">Brindamos lo mejor para nuestros clientes!</p>
            <div class="botones-work">
                <ul>
                    <li class="filter active" data-nombre='todos'>Todos</li>
                    <li class="filter" data-nombre='diseño'>Enterizos Cortos</li>
                    <li class="filter" data-nombre='programacion'>Enterizos Largos</li>
                    <!-- <li class="filter" data-nombre='marketing'>Calzado</li> -->
                </ul>
            </div>
            <div class="galeria-work">
                <div class="cont-work programacion">
                    <div class="img-work"  >
                        <img src="img/Enterizos Largos/1.jpg" alt="">
                    </div>
                    <div class="textos-work">
                        <h4>Producto no disponible - S/ 00.00</h4>
                        
                    </div>
                </div>
                
                <div class="cont-work programacion">
                    <div class="img-work">
                        <img src="img/Enterizos Largos/2.jpg" alt="">
                    </div>
                    <div class="textos-work">
                        <h4>Producto no disponible - S/ 00.00</h4>
                    </div>
                </div>
                <div class="cont-work programacion">
                    <div class="img-work">
                        <img src="img/Enterizos Largos/3.jpg" alt="">
                    </div>
                    <div class="textos-work">
                        <h4>Producto no disponible - S/ 00.00</h4>
                    </div>
                </div>
                <div class="cont-work programacion">
                    <div class="img-work">
                        <img src="img/Enterizos Largos/4.jpg" alt="">
                    </div>
                    <div class="textos-work">
                        <h4>Producto no disponible - S/ 00.00</h4>
                    </div>
                </div>
                <div class="cont-work programacion">
                    <div class="img-work">
                        <img src="img/Enterizos Largos/5.jpg" alt="">
                    </div>
                    <div class="textos-work">
                        <h4>Producto no disponible - S/ 00.00</h4>
                    </div>
                </div>
                <div class="cont-work diseño">
                    <div class="img-work">
                        <img src="img/nodisponible.jpg" alt="">
                    </div>
                    <div class="textos-work">
                        <h4>Producto no disponible - S/ 00.00</h4>
                    </div>
                </div>
                <div class="cont-work diseño">
                    <div class="img-work">
                        <img src="img/nodisponible.jpg" alt="">
                    </div>
                    <div class="textos-work">
                        <h4>Producto no disponible - S/ 00.00</h4>
                    </div>
                </div>
                <div class="cont-work diseño">
                    <div class="img-work">
                        <img src="img/nodisponible.jpg" alt="">
                    </div>
                    <div class="textos-work">
                        <h4>Producto no disponible - S/ 00.00</h4>
                    </div>
                </div>
               <!-- <div class="cont-work marketing">
                    <div class="img-work">
                        <img src="img/shoe1.jpg" alt="">
                        div   </div>
                    <div class="textos-work">
                        <h4>Botas de cuero - S/ 200.00</h4>
                    </div>
                </div>
                <div class="cont-work marketing">
                    <div class="img-work">
                        <img src="img/shoe2.jpg" alt="">
                    </div>
                    <div class="textos-work">
                        <h4>Zapatillas Nike - S/ 357.99</h4>
                    </div>
                </div>
                <div class="cont-work marketing">
                    <div class="img-work">
                        <img src="img/shoe3.jpg" alt="">
                    </div>
                    <div class="textos-work">
                        <h4>Zapatillas Teens - S/ 300.00</h4>
                    </div>
                </div> -->
            </div>
        </section>

<!--OFERTAS-->

    <section class="slide-contenedor" id="ofertas">
         <h3>Ofertas</h3>
         <p class="after"></p>
           <div class="slide-contenedor">
           
            <div class="miSlider">
                <img src="img/oferta2.jpg" alt="">
            </div>
            <div class="miSlider">
                <img src="img/oferta3.jpg" alt="">
            </div>
            <div class="barras">
                <span class="barra active" onclick="posicionSlide(1)"></span>
                <span class="barra" onclick="posicionSlide(2)"></span>
                <span class="barra" onclick="posicionSlide(3)"></span>
            </div>
            <!-- <div class="direcciones">
                <a href="#" class="atras" onclick="avanzaSlide(-1)">&#10094</a>
                <a href="#" class="siguiente" onclick="avanzaSlide(1)">&#10095</a>
            </div> -->
            
        </div>  
        </section> 

<!--Esta seccion es de NUESTRO EQUIPO-->

        <section class="team contenedor" id="equipo">
            <h3>Nuestro equipo</h3>
            <p class="after">Conoce a la gente asombrosa y creativa</p>
            <div class="card">
                <div class="content-card">
                    <div class="people">
                        <img src="img/Junior Lucas Cayetano.jpeg" alt="">
                    </div>
                    <div class="texto-team">
                        <h4>Junior Lucas Cayetano</h4>
                    </div>
                </div>
                <div class="content-card">
                    <div class="people">
                        <img src="img/Isabel Lucia Parco Daga.jpeg" alt="">
                    </div>
                    <div class="texto-team">
                        <h4>Isabel Lucia Parco</h4>
                        
                    </div>
                </div>
                <div class="content-card">
                    <div class="people">
                        <img src="img/Claudia María Saucedo Rodrígue.jpeg" alt="">
                    </div>
                    <div class="texto-team">
                        <h4>Claudia María Saucedo</h4>
                        
                    </div>
                </div>
                <div class="content-card">
                    <div class="people">
                        <img src="img/ALVINO LLANOS, EMANUEL SEBASTIAN.jpeg" alt="">
                    </div>
                    <div class="texto-team">
                        <h4>Emanuel Sebastian Alvino</h4>
                        
                    </div>
                </div>
                <div class="content-card">
                    <div class="people">
                        <img src="img/María Fernanda Villalobos Napán4.jpeg" alt="">
                    </div>
                    <div class="texto-team">
                        <h4>María Fernanda, Villalobos</h4>
                       
                    </div>
                </div>
                
            </div>
        </section>
        
        
    </main>

    <!--Esta seccion es de NUESTROS SERVICIOS-->

    <section class="about" id="servicio">
        <div class="contenedor">
            <h3>Nuestros servicios</h3>
            <p class="after">Siempre mejorando para ti</p>
            <div class="servicios">
                <div class="caja-servicios">
                    <img src="img/stores.svg" alt="">
                    <h4>Productos de calidad</h4>
                    <p>Somos tu tienda favorita SAMI SHOP</p>
                </div>
                <div class="caja-servicios">
                    <img src="img/deliveryy.svg" alt="">
                    <h4>Delivery</h4>
                    <p>Somos tu tienda favorita SAMI SHOP</p>
                </div>
                <div class="caja-servicios">
                    <img src="img/pay.svg" alt="">
                    <h4>100 % legal con Boletas y Facturas</h4>
                    <p>Somos tu tienda favorita SAMI SHOP</p>
                </div>
            </div>
        </div>
    </section>

    
    
    <footer id="contacto">
        
        <script SameSite="None; Secure" src="https://static.landbot.io/landbot-3/landbot-3.0.0.js"></script>
        <script>
            var myLandbot = new Landbot.Livechat({
            configUrl: 'https://chats.landbot.io/v3/H-736869-5WXN18ILQOVJRHJU/index.json',
  });
</script>

    </footer>

    <script src="js/jquery.js"></script>
    <script src="js/main.js"></script>
    <script src="js/filtro.js"></script>
</body>
</html>